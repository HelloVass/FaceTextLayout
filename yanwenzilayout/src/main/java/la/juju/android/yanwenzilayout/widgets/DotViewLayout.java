...
public class DotViewLayout extends LinearLayout implements ViewPager.OnPageChangeListener {
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    mIndicatorSpace = typedArray.getDimensionPixelSize(R.styleable.DotViewLayout_indicator_space,
        mIndicatorSpace);

    // 获取被选中的 indicator 图片资源 ID
    mSelectedIndicatorResId =
        typedArray.getResourceId(R.styleable.DotViewLayout_indicator_selected_src, 0);

    // 获取未选中的 indicator 图片资源 ID
    mUnSelectedIndicatorResId =
        typedArray.getResourceId(R.styleable.DotViewLayout_indicator_unselected_src, 0);
  ...
  private void addIndicator(int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      layoutParams.leftMargin = mIndicatorSpace;
